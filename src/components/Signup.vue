...
export default {
	...
	methods: {
		validate () {
			...
			if (empty) { this.validation.push('All fields are required.') }

			if (this.fields.password !== this.fields.password_confirmation) { this.validation.push('Password and Password Confirmation must match.') }

			if (!this.validation.length) { this.signup() }
		},
		signup () {
