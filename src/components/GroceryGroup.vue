	...
	methods: {
		...
		updateCheckedTotal: function () {
			...
			for (var i in this.$props.area.items) {
				...
				if (item.checked) { this.checkedTotal += (item.qty * item.cost) }
			}
		...
		removeItems: function () {
			for (var i in this.$props.area.items) {
				var item = this.$props.area.items[i]
				if (item.checked) { this.$store.commit('removeItem', { itemIndex: i, areaIndex: this.keyVal }) }
			}
			this.updateCheckedTotal()
		},
		addItem: function () {
			for (var item in this.newItems) {
				...
				i.checked = false
				if (i.name) { this.$store.commit('addItem', { item: i, areaIndex: this.keyVal }) }
			}
