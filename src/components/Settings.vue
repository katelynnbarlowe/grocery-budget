...
export default {
	...
	methods: {
		validate: function () {
			...
			if (empty) { this.validation.push('Names and email are required.') }

			if (this.password !== this.password_confirmation) { this.validation.push('Password and Password Confirmation must match.') }

			if (!this.validation.length) { this.save() }
		},
		async save () {
			this.success = true
			this.error = ''
			...
			if (this.password) { userUpdate.password = this.password }
