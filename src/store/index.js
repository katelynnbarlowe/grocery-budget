		...
		actions: {
			...
			async loadDefaults (context) { // load default settings
				if (context.state.budget === 0) {
					...
					if (value.data) { context.commit('updateBudget', value.data) }
				}
				if (context.state.sales_tax === 0) {
					var value2 = await ListService.getSetting('sales_tax')
					if (value2.data) { context.commit('updateTax', value2.data) }
				}
			},
